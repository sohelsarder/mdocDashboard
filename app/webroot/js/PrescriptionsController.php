...
class PrescriptionsController extends AppController {
	...
	public function add($patient_id = null,$appointment_id =null,$rmp_id = null){ // create prescription 
		...
		if (!$this->$v->hasAny($conditionforPrescription)){
			if ($this->Appointment->hasAny($conditionforAppointment)){				
				...
				if($rmp_info['amount'] >= $unit_conts['UnitCost']['mpower_amount']){	// if rmp credit limit+accountbalance bigger than unit cost price then	prescription created goes			
	...
	function _setValue($appointment=null,$appointment_id =null,$patient_id=null){ // set value for view
		...
		$unit_cost = $this->UnitCost->find('list'); // get unit costs list		
	...
	function _account($dataForSave =null,$total = null,$prescription_id = null,$datasource){ 		
		...
		if(!empty($prescription_id)){
			...
			$update_amount = $accounts['Account']['unitcost'];
		}else{
		...
		if(!empty($total)){ // if prescription paid then goes here
			...
			$current_balance =$rmp_amount['Paramedic']['currentBalance']+$update_amount-0;			
		...
		try{ 
			if($this->Account->save($data)){ // save data into account table
				if($this->Paramedic->updateAll(	array('Paramedic.currentBalance' => $current_balance,'Paramedic.appAmount'=> $app_amount),array('Paramedic.id' => $dataForSave['User']['rmp_id']))){ // update current Balance and appAmount for rmp
					$datasource->commit();
				}else{
					throw new Exception();
				}
	...
	function followUpList($id = null){ //sent followup list to rmp
		...
		where Prescription.is_followup =2 AND Prescription.followupstatus =1 AND Appointment.rmp_id = $id
		AND followupDate between '$pastdate' AND '$predate' group by Prescription.id");
		/*
		$followup_lists = $this->$v->query("SELECT Prescription.id as pid,Prescription.followupDate as followupDate ,Prescription.followupstatus as followupstatus ,Prescription.is_followup as followup,Prescription.appointment_id as appID,Appointment.id as aid,Appointment.rmp_id as rmp
		FROM prescriptions as Prescription left join appointments as Appointment
	    on Prescription.appointment_id = Appointment.id
		where  Prescription.followupstatus = 0 AND Appointment.rmp_id = $id
		"); 
		$followup_lists = $this->$v->find('all',array('conditions'=> array('Prescription.is_followup'=>0)));*/
		echo '<pre>';
		print_r($followup_lists);
		$ret = array();
